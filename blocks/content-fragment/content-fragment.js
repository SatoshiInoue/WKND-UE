...
export default async function decorate(block) {
  ...
  const CONFIG = {
    ...
    GRAPHQL_QUERY: '/graphql/execute.json/automotive-ue/CTAByPath',
  ...
  const persistedquery = '/graphql/execute.json/automotive-ue/CTAByPath';
  ...
  const requestConfig = isAuthor
    ? {
        ...
        };path=${contentPath};variation=${variationname};ts=${Date.now()}`,
        method: 'GET',
  ...
  try {
    ...
    if (isImageLeft) {
      ...
      bannerDetailStyle =
        'background-image: linear-gradient(90deg,rgba(0,0,0,0.6), rgba(0,0,0,0.1) 80%) ,url(' +
        imgUrl +
        ');';
    }
    ...
    block.innerHTML = `<div class='banner-content block ${displayStyle}' data-aue-resource=${itemId} data-aue-label="Offer Content fragment" data-aue-type="reference" data-aue-filter="contentfragment" style="${bannerContentStyle}">
          <div class='banner-detail ${alignment}' style="${bannerDetailStyle}" data-aue-prop="bannerimage" data-aue-label="Main Image" data-aue-type="media" >
                ...
                <div data-aue-prop="description" data-aue-label="Description" data-aue-type="richtext" class='cfdescription'><p>${
                  cfReq?.description?.plaintext || ''
                }</p></div>
                 <p class="button-container ${ctaStyle}">
                  <a href="${
                    cfReq?.ctaUrl ? cfReq.ctaUrl : '#'
                  }" data-aue-prop="ctaUrl" data-aue-label="Button Link/URL" data-aue-type="reference"  target="_blank" rel="noopener" data-aue-filter="page" class='button'>
