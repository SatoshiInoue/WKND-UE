...
export default async function decorate(block) {
  ...
  if (configSrc === 'inline') {
    ...
    if (finalUrl) {
      ...
      finalImg.onerror = function () {
        ...
        // Set fallback image
        this.src =
          'https://smartimaging.scene7.com/is/image/DynamicMediaNA/WKND%20Template?wid=2000&hei=2000&qlt=100&fit=constrain'; // Replace with your fallback image path
  ...
  if (configSrc === 'cf') {
    //https://author-p153659-e1620914.adobeaemcloud.com/graphql/execute.json/automotive-ue/DynamicMediaTemplateByPath;path=
    ...
    const CONFIG = {
      ...
      GRAPHQL_QUERY:
        '/graphql/execute.json/automotive-ue/DynamicMediaTemplateByPath',
    };
    ...
    const contentPath = block
      .querySelector('p.button-container > a')
      ?.textContent?.trim();
    ...
    try {
      ...
      const templateURL =
        offer?.data?.dynamicMediaTemplateByPath?.item?.dm_template;
      const paramPairs =
        offer?.data?.dynamicMediaTemplateByPath?.item?.var_mapping;
